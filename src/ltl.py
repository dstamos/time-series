...
def train_test_meta(data, settings, verbose=True):
    ...
    for regul_param in settings['regul_param_range']:
        ...
        val_performance = multiple_tasks_nmse(val_tasks_test_labels, val_task_predictions, error_progression=False)
            ...
            for task_idx in range(len(all_features)):
                ...
                for curr_regul_param in regul_param:
                    ...
                    for train_index, test_index in kf.split(all_features[task_idx]):
                        x_tr, x_val = all_features[task_idx].iloc[train_index], all_features[task_idx].iloc[test_index]
                        y_tr, y_val = all_labels[task_idx].iloc[train_index], all_labels[task_idx].iloc[test_index]
