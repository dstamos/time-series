...
def train_test_single_task(data, settings):
    ...
    x_merged = pd.concat(features_to_be_merged)
    y_merged = pd.concat(labels_to_be_merged)
    ...
    for regul_param in settings['regul_param_range']:
        ...
        for train_index, test_index in kf.split(x_merged):
            x_tr, x_val = x_merged.iloc[train_index], x_merged.iloc[test_index]
            y_tr, y_val = y_merged.iloc[train_index], y_merged.iloc[test_index]
    ...
    print(f'{"Single task":12s} | test performance: {test_performance:12.5f} | {time() - tt:7.1f}sec')
